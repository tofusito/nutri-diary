...
- Copy yesterday from the day action and choose the meals to bring over, or open a meal's add sheet and use the copy icon to preselect only that meal. The preview skips anything already there, so pressing it twice cannot duplicate entries.
...
Typing is designed around what iOS actually does rather than against it. Sheets that take text fill the screen and are pinned to the layout viewport in plain CSS; the keyboard does not resize that viewport, so nothing moves when it opens. Saving lives in the top bar, out of the keyboard's reach. When a field takes focus it is scrolled up inside the sheet, above any keyboard, so Safari has nothing to pan; if it pans anyway the pan is undone. Script never sizes or positions a sheet from `visualViewport` — that approach raced the keyboard animation and caused the jumping it was meant to prevent. Food and diary fields opt out of Safari's contact, phone and card AutoFill, and the floating tab bar steps aside while a field is focused. `src/lib/keyboard.js` and `src/lib/fields.js` explain each decision and its source. Run `npm run build && npm run check` for Chromium or `BROWSER=webkit npm run check` for WebKit (install it with `npx playwright install webkit`); the checks assert that a changing visual viewport leaves sheets untouched, that low fields are lifted on focus and that no food or diary field is left open to AutoFill. The real iOS keyboard cannot be driven from a test, so the installed PWA still needs a look on an iPhone.
