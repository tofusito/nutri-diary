const CACHE = 'nutri-shell-v3';
self.addEventListener('install', event => {
  event.waitUntil(caches.open(CACHE).then(cache => cache.addAll(['/', '/manifest.webmanifest', '/noodle-180.png', '/noodle-192.png', '/noodle-512.png'])));
});
...
self.addEventListener('fetch', event => {
  ...
  const asset = url.pathname.startsWith('/assets/') || url.pathname.startsWith('/noodle-') || url.pathname.startsWith('/icon') || url.pathname === '/manifest.webmanifest';
