...
export default function Foods({ foods, onFoods }) {
  ...
  const save = async food => { try { const exists = foods.some(item => item.id === food.id); const saved = await api(exists ? `/api/foods/${food.id}` : '/api/foods', { method: exists ? 'PATCH' : 'POST', body: JSON.stringify(food) }); onFoods(exists ? foods.map(item => item.id === food.id ? saved : item) : [saved, ...foods]); setEditing(null) } catch (error) { setMessage(error.message) } }
